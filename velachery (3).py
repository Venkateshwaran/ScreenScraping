...
for eachproperty in soup.findAll('div', {'class':'sT'}):
  ...
  print ' ('
  print today,","+ (a['href'] if a else '`NULL`')+",", (a.string if a else 'NULL, NULL')+ "," +",".join(re.sub(r'([^,]+,[^,]+,)(.*?)([A-Za-z].*)', r'\1\3',re.findall("'([a-zA-Z0-9,\s]*)'", (a['onclick'] if a else 'NULL, NULL, NULL, NULL, NULL, NULL'))))+","+ ", ".join([project] + area),","+pdates+""
  print ' ), '
